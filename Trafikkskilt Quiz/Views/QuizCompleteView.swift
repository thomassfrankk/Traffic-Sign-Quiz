...
struct QuizCompleteView: View {
    ...
    private func evaluateGrandTourCompletion() {
        print("----- evaluateGrandTourCompletion START -----")
        
        guard let currentSettings = settings.first else {
            print("No settings found → exit")
            return
        }
        ...
        guard let countryEntity = countries.first(where: { $0.country == country.rawValue }) else {
            print("No country entity found → exit")
            return
        }
        ...
        print("Evaluating \(sections.count) sections for Grand Tour + Elite")
        print("")
        ...
        outerLoop: for section in sections {
            ...
            if let subs = section.subSections, !subs.isEmpty {
                for sub in subs.sorted(by: { $0.index < $1.index }) {
                    let subName = sub.name(for: userLanguage.rawValue)
                    let score = countryScores.bestScore(for: sectionName, subSection: subName)
                    
                    print("  Subsection: \(subName) → Score: \(score)")
                    
                    if score < 80 {
                        print("    → score < 80 → Normal GT incomplete")
                        allAtLeast80 = false
                    }
                    if score < 100 {
                        print("    → score < 100 → Elite incomplete")
                        allAt100 = false
                    }
                    
                    if !allAtLeast80 && !allAt100 {
                        print("    → Both flags false → stopping evaluation early")
                        break outerLoop
                    }
                }
            } else {
                let score = countryScores.bestScore(for: sectionName)
                print("  No subsections → Score: \(score)")
        ...
        print("Final status: allAtLeast80 = \(allAtLeast80), allAt100 = \(allAt100)")
        print("")
        
        // Optional requirement text
        if allAtLeast80 {
            grandTourRequirement = "Elite requires 100%"
            print("Grand Tour requirement text updated (user completed GT)")
        }
        
        // Normal Grand Tour
        if countryScores.isGrandTourCompleted != allAtLeast80 {
            print("Updating isGrandTourCompleted → \(allAtLeast80)")
            countryScores.isGrandTourCompleted = allAtLeast80
            isGrandTourCompleted = allAtLeast80
        } else {
            print("isGrandTourCompleted unchanged (\(countryScores.isGrandTourCompleted))")
        }
        
        // Elite Grand Tour
        if countryScores.isGrandTourEliteCompleted != allAt100 {
            print("Updating isGrandTourEliteCompleted → \(allAt100)")
            countryScores.isGrandTourEliteCompleted = allAt100
            isGrandTourEliteCompleted = allAt100
        } else {
            print("isGrandTourEliteCompleted unchanged (\(countryScores.isGrandTourEliteCompleted))")
        }
        
        print("----- evaluateGrandTourCompletion END -----")
    }
...
//    }
    ...
    var body: some View {
                ...
                VStack(spacing: isSmall ? 16 : 20) {
                    ...
                    Button {
                        if result.gamePassed, quizMode == .grandTour {
                            if isGrandTourEliteCompleted {
                                showResetTourWarning = true
                            } else {
                                router
                                    .goToQuizIntro(
                                        mode: .grandTour,
        ...
        .onChange(of: showResults) { _, newValue in
            guard let settings = settings.first, newValue else { return }
            
            // Play sound only after results are shown
            if settings.soundEnabled {
                if quizMode == .grandTour && result.gamePassed {
                    SoundManager.playSound(named: "tada")
                } else if quizMode != .grandTour && (isNewHigh || correctAnswers == best || score == best) {
                    SoundManager.playSound(named: "tada")
        ...
        .alert(
            languageManager.localizedString(for: "Restart Grand Tour?"),
            isPresented: $showResetTourWarning
        ) {
            Button(languageManager.localizedString(for: "Restart"), role: .destructive) {
                restartTournament()
            }
            Button(languageManager.localizedString(for: "Cancel"), role: .cancel) { }
        } message: {
            Text(languageManager.localizedString(
                for: "This will erase all section and subsection high scores and reset your tour progress. This action cannot be undone."
            ))
        }
    }
    ...
    func restartTournament() {
        guard
            ...
            let countryEntity = countries.first(where: { $0.country == country.rawValue })
        else { return }
        ...
        for section in sections {
            ...
            if let subs = section.subSections, !subs.isEmpty {
                for sub in subs.sorted(by: { $0.index < $1.index }) {
                    let subName = sub.name(for: userLanguage.rawValue)
                    countryScores.resetSubSectionScore(for: sectionName, subSection: subName)
                }
            } else {
                countryScores.resetSectionScore(for: sectionName)
            }
        }
        
        // Reset global flags
        countryScores.isGrandTourCompleted = false
        countryScores.isGrandTourEliteCompleted = false
        isGrandTourCompleted = false
        isGrandTourEliteCompleted = false
    ...
    private func saveAndCheckHighScore() {
        print("running saveAndCheckHighScore")
