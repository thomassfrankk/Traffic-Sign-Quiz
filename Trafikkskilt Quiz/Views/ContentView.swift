...
//        print("User language requested: \(userLanguage.rawValue)")
        
        // If user requests English, normalize to en-GB
        let normalizedUserLanguage: Language = userLanguage.rawValue.starts(with: "en") ? .englishGB : userLanguage
        
        // If the normalized language is supported, use it
        if country.supportedLanguages.contains(normalizedUserLanguage) {
//            print("Supported language: \(normalizedUserLanguage)")
            return normalizedUserLanguage
        }
        
        // Otherwise, pick the first fully supported language
        var fallback = country.supportedLanguages.first { $0.isFullySupported } ?? .englishGB
        
        // If user requested English, override fallback to en-GB
        if userLanguage.rawValue.starts(with: "en") {
            fallback = .englishGB
        }
        
//        print("Supported language (general fallback): \(fallback)")
        return fallback
    }
    ...
    private func canonicalLanguageKey(for language: String) -> String {
        switch language {
        case let lang where lang.starts(with: "en"):
            return "en-GB"   // treat all English variants the same
        default:
            return language
        }
    }
    ...
    private func hasCompleteTranslation() -> Bool {
        let key = canonicalLanguageKey(for: userLanguage.rawValue)
        
        guard let countryLanguage = countryEntity?.language[key] else {
            print("❌ Language not found. Looking for: \(key)")
        ...
        print("✅ Found language: \(key)")
        print("Has completed translation: \(!countryLanguage.generalInformation.isEmpty)")
    ...
    var body: some View {
        NavigationStack(path: $router.path) {
            VStack(spacing: 0) {
//                Text(userLanguage != supportedLanguage ? displayCountry ?? "" : country.localizedName)
                Text(userLanguage != supportedLanguage ? displayCountry ?? "" : userLanguage.rawValue.hasPrefix("en") ? country.rawValue : country.localizedName)
                    ...
                    .translationTask(configuration) { session in
                        if userLanguage.rawValue.prefix(2) != supportedLanguage.rawValue.prefix(2),
                            !showingSettings,
                            !showOnboarding,
                            !isUpdating {
                            do {
...
//                    .id("\(languageDownloadedTapped), \(showingSettings), \(showOnboarding), \(isUpdating)")
                    .id("\(country.rawValue), \(languageDownloadedTapped), \(showingSettings), \(showOnboarding), \(isUpdating)")
                    .transition(.opacity)
                ...
                ToolbarItem(placement: .subtitle) {
                    if let syncMessage = syncMessage {
                        Text(syncMessage)
                            ...
                            .id(syncMessage)
                            .transition(.opacity)
                            .padding(.leading, isPad ? 0 : isSmall ? 8 : 16)
                    }
                }
...
//            .fullScreenCover(isPresented: .constant(true)) {
        ...
        .onAppear {
            InactivityReminderManager.shared.scheduleInactivityReminder(languageManager: languageManager)
        ...
        Task { @MainActor in
            ...
            do {
                for try await response in session.translate(batch: requests) {
                    ...
                    case "section-sign-section-name":
                        guard let sectionIndex = path.sectionIndex,
                              let signIndex = path.signIndex,
                              let section = entity.sections.first(where: { $0.index == sectionIndex }),
                              signIndex < section.signs.count else { continue }
                        if section.signs[signIndex].language[langCode] == nil {
                            section.signs[signIndex].language[langCode] = SignLanguageContent(title: "", details: "", wrongAnswers: [], sectionName: response.targetText, subSectionName: nil)
                        } else {
                            section.signs[signIndex].language[langCode]?.sectionName = response.targetText
                        }
                        
                    case "section-sign-subsection-name":
                        guard let sectionIndex = path.sectionIndex,
                              let signIndex = path.signIndex,
                              let section = entity.sections.first(where: { $0.index == sectionIndex }),
                              signIndex < section.signs.count else { continue }
                        if section.signs[signIndex].language[langCode] == nil {
                            section.signs[signIndex].language[langCode] = SignLanguageContent(title: "", details: "", wrongAnswers: [], sectionName: "", subSectionName: response.targetText)
                        } else {
                            section.signs[signIndex].language[langCode]?.subSectionName = response.targetText
                        }
                        
                    case "subsection-sign-section-name":
                        guard let sectionIndex = path.sectionIndex,
                              let subsectionIndex = path.subsectionIndex,
                              let signIndex = path.signIndex,
                              let section = entity.sections.first(where: { $0.index == sectionIndex }),
                              ...
                              signIndex < subsection.signs.count else { continue }
                        if subsection.signs[signIndex].language[langCode] == nil {
                            subsection.signs[signIndex].language[langCode] = SignLanguageContent(title: "", details: "", wrongAnswers: [], sectionName: response.targetText, subSectionName: nil)
                        } else {
                            subsection.signs[signIndex].language[langCode]?.sectionName = response.targetText
                        }
                        
                    case "subsection-sign-subsection-name":
                        guard let sectionIndex = path.sectionIndex,
                              let subsectionIndex = path.subsectionIndex,
                              let signIndex = path.signIndex,
                              let section = entity.sections.first(where: { $0.index == sectionIndex }),
                              ...
                              signIndex < subsection.signs.count else { continue }
                        if subsection.signs[signIndex].language[langCode] == nil {
                            subsection.signs[signIndex].language[langCode] = SignLanguageContent(title: "", details: "", wrongAnswers: [], sectionName: "", subSectionName: response.targetText)
                        } else {
                            subsection.signs[signIndex].language[langCode]?.subSectionName = response.targetText
                        }
