...
//                        .padding(.horizontal)
                        ...
                        if expandedSubSectionIDs.contains(sub.id) {
                            ForEach(Array(signs.enumerated()), id: \.element.id) { index, sign in
                                SignCardView(sign: sign, isPad: isPad, isSmall: isSmall, purchaseManager: purchaseManager, languageManager: languageManager, configuration: configuration, userLanguage: userLanguage, supportedLanguage: supportedLanguage, country: country)
...
struct SignCardView: View {
    ...
    let multiSigns = [
        ...
        "No Parking Except Electric Vehicles / Charging Only",
        "Directional Object Marker",
        "School Zone Speed Limit",
        "Push Button for Pedestrian Signal",
        "Aproximación a un paso a nivel, puente móvil o muelle"
    ]
        ...
        VStack(spacing: 32) {
            ...
            if uiImages.count == 1, let image = uiImages.first {
                Image(uiImage: image)
                    .resizable()
                    .scaledToFit()
                    .frame(maxHeight: isPad ? 200 : isSmall ? 120 : 140)
            ...
            else if multiSigns.contains(sign.title(for: country.supportedLanguages.first!.rawValue)) {
                HStack(spacing: 0) {
