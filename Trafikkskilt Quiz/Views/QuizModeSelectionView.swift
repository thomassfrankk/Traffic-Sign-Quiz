...
struct QuizModeSelectionView: View {
    ...
    func findFirstIncompleteSection(isExtreme: Bool = false) -> (section: TrafficSectionEntity, subsection: TrafficSubSectionEntity?)? {
        ...
        for section in sortedSections {
            ...
            if let subSections = section.subSections, !subSections.isEmpty {
                for subsection in subSections {
                    ...
                    if isExtreme {
                        // Elite mode → MUST be 100% to be complete
                        if score < 100 {
                            return (section, subsection)
                        }
                    } else {
                        // Normal mode → 80%+ is complete
                        if score < 80 {
                            return (section, subsection)
                        }
                    }
            ...
            ScrollView(showsIndicators: false) {
                VStack(spacing: 0) {
                    ...
                    VStack(spacing: 16) {
                        ...
                        let firstIncomplete = findFirstIncompleteSection(isExtreme: false)
                        let nextSection = firstIncomplete ?? findFirstIncompleteSection(isExtreme: true)
                        let progress = firstIncomplete != nil ?
                        calculateOverallProgress(isExtreme: false) :
                        calculateOverallProgress(isExtreme: true)
                        
                        QuizModeCard(
                            title: firstIncomplete != nil ? "Grand Tour" : "Grand Tour Elite",
                            description: firstIncomplete != nil ? "All signs. Section by section. Limited lives." : "All signs. Section by section. One life.",
                            icon: "car.circle",
                            ...
                            action: { withAnimation { quizMode = .grandTour } }, // action closure
                            onReset: { restartTournament() }                     // onReset closure
                        )
                        ...
                        QuizModeCard(
                            ...
                            overallProgress: 0,
                            action: { withAnimation { quizMode = .grandPrix } },
                            onReset: nil  // must provide nil explicitly
                        )
    ...
    var body: some View {
        VStack(alignment: .leading) {
            ...
            if isSelected {
                HStack(spacing: 16) {
                    Button {
                        if nextIncompleteSection == nil, quizMode == .grandTour {
